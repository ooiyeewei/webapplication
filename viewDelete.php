...
function validateForm() {
    var x = document.forms["deleteForm"]["delete"].value;

    if (x == "") {
        alert("Error! User need to choose a movie to delete.");
        return false;
    }
}
</script>
		...
		<div class="title">
			<h2>Delete Movie</h2>
				...
				<table class="data-table">
					...
					<tbody>
					<form name="deleteForm" method="post" onsubmit="validateForm()">
						<?php
							while ($row = $result->fetch_assoc())
							{
								...
								<td>'.$row['genre'].'</td>
								<td>'.$row['years'].'</td>
								<td><input type="radio" name="delete" value="'.$row['id'].'"></td>
								</tr>';
							}
						?>
					</tbody>
